//El telefono tendra el (+34) 666 666 666
let telefono = /^\(\+[0-9]{1,3}\)[0-9]{9}$/
//Tentra contra barra parentesis contra barra + para que salga el parentesis y el mas
//Del 0 al nueve un total de 1 a 3 y 9 numeros

//Hora  tendra del 0 al 2 junto el 0 al 3 o del 0 al 1 junto 0 al 9 los : y los mins
let hora = /^(([0-2][0-3])|([0-1][0-9])):[0-5][0-9])$/

//El mail
let email = /^[^áàéèíìïóòúùü\s\(\)<>@,;:\"\[\]ç%&]{1,64}@[^áàéèíìïóòúùü\s\(\)<>@,;:\"\[\]ç%&]{1,251}.(com|es|net)$/;
//Tendra el nombre cualquiera con la @ seguido del dominio que quiera mas el . net .com y otro dominio antes

let web = /^http:\/\/www\.[^\.]+\..{3}$/;
//Tendra http://www -> obligado seguido del punto (contrabarra para poder usarlo)
//Podra llebar otro punto y punto con el final com es ....
